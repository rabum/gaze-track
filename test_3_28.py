...
def get_video_acc(video_path, inferencer, yolo_model):
    ...
    for i in range(total_frames):
        ...
        #cv2.imwrite(f"temp_wrong2/{i}.jpg", frame)
...
if __name__ == "__main__":
    ...
    video_path = 'open_door/right-3-26/23f7feb541454571c502bb38f84cdcfd.mp4'
    print(get_video_acc(video_path, inferencer, yolo_model))
    assert 0
    #video_path = 'open_door/wrong-3-26/1b24c7f3dba197682eeb983d3f530e9c.mp4'
    files_list = []
    for folder in ['right-3-26', 'wrong-3-26']:
        for file in os.listdir(f'open_door/{folder}'):
            files_list.append(f'open_door/{folder}/{file}')
    final_acc = 0
    with open('log.txt', 'a', buffering=1) as f:
        for file in tqdm(files_list):
            ...
            f.write(file + '\t')
            try:
                acc = get_video_acc(file, inferencer)
                if acc == -1:#无论是right还是wrong，预测结果均为wrong
                    if label == 'wrong':
                        f.write('wrong\n')
                        final_acc += 1
                    else:
                        f.write('wrong(no valid frame)\n')
                    continue
                if label == 'right':
                    if acc > 0.5: #超过一半的双人帧均注视
                        f.write(f'right(acc {acc})\n')
                        final_acc += 1
                    else:
                        f.write(f'wrong(acc {acc})\n')
                else:
                    if 0 < acc <= 0.5:
                        f.write(f'wrong(acc {acc})\n')
                        final_acc += 1
                ...
                f.write('needcheck\n')
    print(final_acc / len(files_list))